...
kleb_eth_death <- left_join(eth11,
                          kleb_death_eth_num,
...
#table for poisson
kleb_ethnicity_poisson <- kleb_ethrates_table %>% 
...
kleb_ethnpoisson <- glm(cases ~ group,
                      offset = log(livebirths),
                      #family = quasipoisson(link = "log"),
                      family = poisson(link = "log"),
                      data = kleb_ethnicity_poisson)

kleb_eth_poisson_tbl <- gtsummary::tbl_regression(kleb_ethnpoisson, exponentiate = TRUE)


kleb_eth_tbl_custom <- kleb_eth_poisson_tbl %>%
  modify_table_body(
    ~ .x %>%
      mutate(
        `IRR (95% CI)` = ifelse(
