...
kleb_deathrates_tbl <- kleb_rates %>% 
...
##summary stats
kleb_rate_numerator <- kleb_final %>% 
  filter(address %in% c("East of England",
                        "East Midlands",
                        "London",
                        "North East",
...
##now for deaths
kleb_death_numerator <- kleb_final %>% 
  filter(address %in% c("East of England",
                        "East Midlands",
                        "London",
                        "North East",
...
kleb_total_rates <- left_join(kleb_rate_numerator,
                         kleb_pseu_region_denominator,
                         by = c("address" = "geo_name"))
kleb_rate_final <- left_join(kleb_total_rates,
                           kleb_death_numerator,
                           by = "address")

kleb_totalrates <- kleb_rate_final %>% 
...
kleb_pop_rates<- kleb_totalrates %>% 
  select(address,
         cases,
         case_rate,
         deaths,
...
save_as_docx(
  "Regional trends in Klebsiella bacteremia among infants in England: incidence, mortality and case-fatality rates for the period 2011-2019" = kleb_flexreg,
  "Trends in Klebsiella bacteremia among infants in England: incidence, mortality and case-fatality rate for the period 2011-2019 by mother ethnicity" = kleb_flexteh,
  path = here("outputs", "kleb_rates_tables.docx"))
