...
kleb_gestage_join <- left_join(kleb_gestage,
                             kleb_gestage_death,
                             by = "gestage_grp")
kleb_gestage_join <- left_join(kleb_gestage_join,
                             kleb_pseu_gestage,
                             by = c("gestage_grp"="gestage_group"))

kleb_gestage_join <- kleb_gestage_join %>% 
...
#table for poisson
kleb_gestage_poisson <- kleb_gestage_join %>%
  mutate(
...
kleb_gestagepoisson <- glm(cases ~ gestage_grp,
                         offset = log(total_births),
                         #family = quasipoisson(link = "log"),
                         family = poisson(link = "log"),
                         data = kleb_gestage_poisson)


kleb_gest_poisson_tbl <- gtsummary::tbl_regression(kleb_gestagepoisson, exponentiate = TRUE)


kleb_gest_tbl_custom <- kleb_gest_poisson_tbl %>%
  modify_table_body(
    ~ .x %>%
      mutate(
        `IRR (95% CI)` = ifelse(
