#### ethnicity rates for klebsiella
...
kleb_eth_live$live_births <- as.numeric(gsub(",","", kleb_eth_live$live_births))



kleb_eth14 <- kleb_eth_live %>%   
  group_by(ethnicity) %>% 
...
kleb_eth_rate <- left_join(kleb_ethnos,
                         kleb_eth14,
                         by = "ethnicity")

kleb_eth_rate <- kleb_eth_rate %>% 
  mutate( n = as.numeric(n),
          case_rate = (n/population_ethn14)*100000,
          case_rate = round(case_rate, digits=2))

##now for death rates
kleb_death_eth_num <- kleb_final %>% 
  filter(death_30 == 1) %>% 
  drop_na(any_of(c("ethnicity")
  )
  ) %>% 
...
kleb_eth_death <- left_join(kleb_eth14,
                          kleb_death_eth_num,
                          by = c("ethnicity"))

kleb_eth_death <- kleb_eth_death %>% 
...
kleb_ethtable <- left_join(kleb_eth_death,
                         kleb_eth_rate,
                         by= c("ethnicity"))


kleb_ethtable <- kleb_ethtable %>% 
  mutate(case_fatality = (deaths/cases)*100,
         case_fatality = round(case_fatality, 1))

kleb_ethrates_table <- kleb_ethtable %>% 
  select(ethnicity,
         cases,
         case_rate,
         deaths,
...
kleb_flexeth <- flextable(klebethtable)


border_style = officer::fp_border(color="black", width=1)

kleb_flexteh <- kleb_flexeth %>% 
  autofit() %>% 
  add_header_row(
    top = TRUE,
    values = c("",
...
print(kleb_flexteh, preview = "docx")
