...
pseu_regtable <- pseu_rates_table %>% 
  select(
    address,
    total_births,
    cases,
...
pseu_pop_rates<- pseu_rates_table %>% 
  select(address,
         cases,
         case_rate,
         deaths,
...
pseu_region_poisson <- pseu_regtable %>% 
...
pseu_regpoisson <- glm(cases ~ address,
                       offset = log(total_births),
                       #family = quasipoisson(link = "log"),
                       family = poisson(link = "log"),
                       data = pseu_region_poisson)

pseu_reg_poisson_tbl <- gtsummary::tbl_regression(pseu_regpoisson, exponentiate = TRUE)


pseu_reg_tbl_custom <- pseu_reg_poisson_tbl %>%
  modify_table_body(
    ~ .x %>%
      mutate(
        `IRR (95% CI)` = ifelse(
