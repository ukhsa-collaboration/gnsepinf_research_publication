...
#07/04/2025
# we will define our outcome of interest as death 30 days post onset
# all cohort together
# birthweight over gestational age because of higher completion
#
...
ec_univariate_paper <- ec_univariate_paper %>% 
...
table(ec_univariate_paper$death_30)
...
ec_univariate_paper_presentation <- ec_univariate_paper %>%
  mutate(
...
ec_univ_tab_paper <- ec_univariate_paper_presentation %>% 
  #ec_univariate3 %>% 
  dplyr::select(ec_explanvars_paper, death_30) %>% ## select variables of interest
