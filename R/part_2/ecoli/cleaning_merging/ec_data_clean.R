###Data cleaning of the EColi dataset
...
ec_clean <- ec %>%
  ...
  mutate(ethnicity_simple = case_when(
    ethnicity_des2 %in% c("South Asian",
                          "Asian other") ~ "Asian",
    ethnicity_des2 %in% c("Other ethnicity",
                          "Mixed") ~ "Mixed and other ethnicity",
    TRUE ~ ethnicity_des2
  )) %>% 
  ...
  mutate(
    delivery_location = case_when(
      delplac_1 %in% c(0,2,3,4,7,5,6,8) ~ "NHS hospital",
  ...
  mutate(
    ethnicity_simple = case_when(
      ethnicity_simple == "Not states" ~ NA_character_,
      TRUE ~ ethnicity_simple
    )
  ) %>%
  mutate(
