...
EC1_hes_collapsed <- EC1_hes_collapsed %>% rename(patient_id=id)

EC1_hes_collapsed <- EC1_hes_collapsed %>%  mutate_at(vars(spell_end_date),~ymd(.)) %>% 
                                            mutate_at(vars(dob),~ymd(.)) %>% 
                                            mutate_at(vars(spell_start_date),~ymd(.)) 


Data1 <- Data1 %>% select(nhs, dob_br, data_collection_date,id) %>% 
...
link1 <- inner_join(Data1,EC1_hes_collapsed,
                    by=c("nhs"="newnhsno",
                         "dob_br"="dob"),
                    na_matches="never")
...
nomatch1 <- anti_join(Data1,EC1_hes_collapsed,
                    by=c("nhs"="newnhsno",
                         "dob_br"="dob"),
                    na_matches="never")
...
#Remove created variable for downstream analysis
...
write.csv(inhos,paste(path_out,'Linkage_Ecoli_DCS_infantHES_Success_inhospital_New Linkage.csv',sep = ''), row.names = FALSE)
write.csv(inhos2,paste(path_out,'Linkage_Ecoli_DCS_infantHES_Success_inhospital_OR_admitted_1day_postbac_New Linkage.csv',sep = ''), row.names = FALSE)
write.csv(notinhos_summary,paste(path_out,'Output_table_NotinHospital_Ecoli_DCS_infantHES_New Linkage.csv',sep = ''), row.names = FALSE)
