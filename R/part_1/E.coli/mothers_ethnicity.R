...
EC1_BR_mHES <- EC1_BR_mHES %>% mutate(ethnos=as.character(ethnos)) 

EC1_BR_mHES <- EC1_BR_mHES %>% mutate(ethnicity_grp=case_when(ethnos %in% c("A ") ~ " White British",
...
data_3.2E <- EC1_BR_mHES %>% group_by(ethnicity_grp) %>% tally()
