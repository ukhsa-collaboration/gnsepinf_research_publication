...
KS1 <- KS1 %>% mutate(specimen_date=as.Date(specimen_date, format = "%d/%m/%Y")) %>% 
...
#Rename
KS1 <- KS1 %>% ungroup() %>%  rename(nhs=nhs_number,
                                   hos=hospital_number,
                                   name1=forename,
                                   name2=surname,
                                   sex=sex,
...
KS1 <- KS1 %>% mutate_at(vars(name1, name2),~(toupper(.)))

#Validity check

KS1 <- KS1 %>% mutate(nhs=as.numeric(nhs))
nhsvalid <-valid_nhs(KS1$nhs)
nhsvalid <- nhsvalid %>% mutate(nhs_number=as.numeric(nhs_number))
KS1 <- left_join(KS1, nhsvalid, by = c("nhs" = "nhs_number"), keep=FALSE)

KS1 <- KS1 %>% mutate(hos_unknown=hos %in% c("UNKNOWN","NO PATIENT ID",NA)) %>% 
               mutate(dob_unknown=dob %in% c("1900-01-01",NA))


## Create missing data summary output

missing1 <- KS1 %>% select(dob_unknown, hos_unknown) %>% summarise_all(~sum(.))
missing2 <- KS1 %>% filter(valid_nhs == "FALSE") %>%  tally() %>% rename(nhs_invalid=n)
...
KS1 <- KS1 %>% mutate(age=(date-dob)) %>% filter(age <= 365)


##Save file

write.csv(KS1,paste(path_out,'DSC_Klebsiella_Extract.csv',sep = ''))
