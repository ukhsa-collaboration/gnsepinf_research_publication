...
KS1_hes_collapsed <- KS1_hes_collapsed %>% rename(patient_id=id)

KS1_hes_collapsed <- KS1_hes_collapsed %>%  mutate_at(vars(spell_end_date),~ymd(.)) %>% 
                                            mutate_at(vars(dob),~ymd(.)) %>% 
                                            mutate_at(vars(spell_start_date),~ymd(.)) 



KS1_BR <- KS1_BR %>% select(nhs, dob_br, data_collection_date,id) %>% 
...
link1 <- inner_join(KS1_BR,KS1_hes_collapsed,
                    by=c("nhs"="newnhsno",
                         "dob_br"="dob"),
                    na_matches="never")
...
nomatch1 <- anti_join(KS1_BR,KS1_hes_collapsed,
                    by=c("nhs"="newnhsno",
                         "dob_br"="dob"),
                    na_matches="never")
...
link1<- link1 %>% mutate(
  ...
  mutate(
    ...
    birth_episode=dob_br==spell_start_date,spell_end_date)
...
#path_out = "//filepor10.phe.gov.uk/pid_hes$/Colindale/HCAI/Pfizer Infant Gram-negative/R/Linkage (iHES-DSC)/"
#write.csv(inhos,paste(path_out,'Linkage_Klebsiella_DCS_infantHES_Success_inhospital.csv',sep = ''), row.names = FALSE)
#write.csv(inhos2,paste(path_out,'Linkage_Klebsiella_DCS_infantHES_Success_inhospital_OR_admitted_1day_postbac.csv',sep = ''), row.names = FALSE)
#write.csv(notinhos_summary,paste(path_out,'Output_table_NotinHospital_Klebsiella_DCS_infantHES.csv',sep = ''), row.names = FALSE)
