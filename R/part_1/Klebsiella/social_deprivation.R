...
KS1_BR_mHES <- KS1_BR_mHES %>% mutate(imd04_grp=case_when(imd04rk<=6568 ~ "1st",
...
data_3.2F <- KS1_BR_mHES %>% group_by(imd04_grp) %>% tally()

#Examining records that have a gestational age of <22 weeks

data <- KS1_BR_mHES %>% filter(imd04_grp=="Unknown") %>% select(imd04,imd04rk,imd04_grp)