...
##Pathogen: Klebsiella sp.
##ProjKSt: WP3 infant spesis
##Created by: Luisa Hallmaier-Wacker
source("path/path.R")

KS1_hes <- KS1_hes %>% clean_names()

date_fields <- KS1_hes %>% select(contains("date")) %>% names()
diag_fields <- KS1_hes %>% select(contains("diag")) %>% names()
pro_fields <- KS1_hes %>% select(contains("procode")) %>% names()
pii_fields <- c("newnhsno","valid_nhs","lopatid","dob",
                "sex","ethnos","rescty","homeadd")

## FIELD MAINTENENCE ############################################################
## clean fields
# regular attenders have  elKStive day/night admissions, as part of regular ongoing care
...
## GROUP PATIENT RKSORDS ########################################################
KS1_hes <- patient_id(data=KS1_hes,
                  nhs_number = "newnhsno",
...
## group SUS into CIPs [requires sense chKSk]
...
## Due to COVID19 HCAI attribution methods, it was dKSided that CIPs where a
...
## slow but works
for(i in 1:max(KS1_hes2$cip_counter)){
...
KS1_hes3 <- left_join(spell_data, diagnostic_icd10,
                 by = c("id",
                        "provider_code",
...
KS1_hes4 <- left_join(spell_data, proceedure_icd10,
                      by = c("id",
                             "provider_code",
                             "hospital_provider_spell_no")
)
...
KS1_hes5 <- left_join(KS1_hes3, KS1_hes4,
                      by = c("id",
                             "mega_spell_id")
)
...
KS1_hes_collapsed <- right_join(pid,KS1_hes5,
                  by=c("id",
                       "provider_code",
                       "mega_spell_id")) %>% 
  distinct(id, mega_spell_id, .keep_all = T)
...
write.csv2(KS1_hes_collapsed,paste(path_out,'DSC_Klebsiella_HES_spells_collapsed_concat_proceedures_diagnostic_codes.csv',sep = ''), row.names=FALSE)

##################################################################################################

##Produce output for HES rKSords (non-collapsed spells with individual episodes per row)
...
#Note that there are HES rKSords from NHS numbers that we have identified as non-infants 
#

## collect spell data
KS1_hes_full <- KS1_hes2 %>%
  ungroup() %>% 
  select(
    id,
    procode3,
...
KS1_hes_diag <- left_join(KS1_hes_full, diagnostic_icd10,
                      by = c("id",
                             "provider_code",
                             "hospital_provider_spell_no"
                             )
...
KS1_hes_proc <- left_join(KS1_hes_full, proceedure_icd10,
                          by = c("id",
                                 "provider_code",
                                 "hospital_provider_spell_no")
)
...
write.csv2(KS1_hes_diag,paste(path_out,'DSC_Klebsiella_HES_spells_NOTcollapsed_diagnostic_codes_for_episodes.csv',sep = ''), row.names=FALSE)
write.csv2(KS1_hes_proc,paste(path_out,'DSC_Klebsiella_HES_spells_NOTcollapsed_proceedures_codes_for_episodes.csv',sep = ''), row.names=FALSE)


##Clean-up
rm(KS1_hes1, KS1_hes2, KS1_hes3, KS1_hes4, KS1_hes5,pid, proceedure_icd10, spell_data, KS1_hes, diagnostic_icd10)
