...
PA1_hes <- PA1_hes %>% clean_names()

date_fields <- PA1_hes %>% select(contains("date")) %>% names()
diag_fields <- PA1_hes %>% select(contains("diag")) %>% names()
pro_fields <- PA1_hes %>% select(contains("procode")) %>% names()
...
PA1_hes <- patient_id(data=PA1_hes,
                  nhs_number = "newnhsno",
...
## slow but works
for(i in 1:max(PA1_hes2$cip_counter)){
...
PA1_hes3 <- left_join(spell_data, diagnostic_icd10,
                 by = c("id",
                        "provider_code",
...
PA1_hes4 <- left_join(spell_data, proceedure_icd10,
                      by = c("id",
                             "provider_code",
                             "hospital_provider_spell_no")
)
...
PA1_hes5 <- left_join(PA1_hes3, PA1_hes4,
                      by = c("id",
                             "mega_spell_id")
)
...
PA1_hes_collapsed <- right_join(pid,PA1_hes5,
                  by=c("id",
                       "provider_code",
                       "mega_spell_id")) %>% 
  distinct(id, mega_spell_id, .keep_all = T)
...
write.csv2(PA1_hes_collapsed,paste(path_out,'DSC_Paeruginosa_HES_spells_collapsed_concat_proceedures_diagnostic_codes.csv',sep = ''), row.names=FALSE)
...
PA1_hes_diag <- left_join(PA1_hes_full, diagnostic_icd10,
                      by = c("id",
                             "provider_code",
                             "hospital_provider_spell_no"
                             )
...
PA1_hes_proc <- left_join(PA1_hes_full, proceedure_icd10,
                          by = c("id",
                                 "provider_code",
                                 "hospital_provider_spell_no")
)
...
write.csv2(PA1_hes_diag,paste(path_out,'DSC_Paeruginosa_HES_spells_NOTcollapsed_diagnostic_codes_for_episodes.csv',sep = ''), row.names=FALSE)
write.csv2(PA1_hes_proc,paste(path_out,'DSC_Paeruginosa_HES_spells_NOTcollapsed_proceedures_codes_for_episodes.csv',sep = ''), row.names=FALSE)


##Clean-up
rm(PA1_hes1, PA1_hes2, PA1_hes3, PA1_hes4, PA1_hes5,pid, proceedure_icd10, spell_data, PA1_hes, diagnostic_icd10)
