...
PA1_BR_mHES <- PA1_BR_mHES %>% mutate(ethnos2=as.character(ethnos))

PA1_BR_mHES <- PA1_BR_mHES %>% mutate(ethnicity_grp=case_when(ethnos %in% c("A ") ~ " White British",
...
data_3.2E <- PA1_BR_mHES %>% group_by(ethnicity_grp) %>% tally()
