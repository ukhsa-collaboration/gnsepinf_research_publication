...
PA1_BR_mHES <- PA1_BR_mHES %>% mutate(birth_weight_equal=case_when(birthwgt_br==birweit_1 | birthwgt_br==birweit_2 | birthwgt_br==birweit_3~1,
                                                                   TRUE ~ 0)) %>% 
                               mutate(birth_weight_equal=as.integer(birth_weight_equal)) 



PA1_BR_mHES <- PA1_BR_mHES %>% mutate(birthwgt_combined=case_when(birth_weight_equal=="1" ~1,
...
PA1_BR_mHES <- PA1_BR_mHES %>% mutate(birthwgt_fin=case_when(birthwgt_combined=="1" ~ birthwgt_br,
                                                                  birthwgt_combined=="2" ~ birweit_1))




PA1_BR_mHES <- PA1_BR_mHES %>% mutate(birthwgt_grp=case_when(birthwgt_fin < 1000 ~ "Extremely Low",
...
data_3.2C <- PA1_BR_mHES %>% group_by(birthwgt_grp) %>% tally()
