...
PA_vit<- PA_vit %>% filter(RecType %in% c(30, 33))


PA_vit <- PA_vit %>% mutate_at(vars(data_collection_date), ~dmy(.)) %>% 
                    mutate_at(vars(DateOfDeath), ~ymd(.)) %>% 
                    mutate(baby_dead=if_else(is.na(DateOfDeath), 0, 1))

#Filter for dead babies
PA_vit_D <- PA_vit %>% filter(baby_dead==1) %>% mutate(diff_time_death_bact=difftime(
                        DateOfDeath, data_collection_date, units = "days")) %>%
                        mutate_if(is.difftime,as.integer)



PA_vit_D <- PA_vit_D %>%  mutate(diff_time_death_bact_grp=case_when(diff_time_death_bact %in% c(0,1,2,3,4,5,6,7) ~ "0-7 days",
...
data_3.3E <- PA_vit_D %>% group_by(diff_time_death_bact_grp) %>% tally()